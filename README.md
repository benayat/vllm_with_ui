...
```json
{
  ...
  "post_processors": [
    ...
    {"name": "python_script", "config": {"entrypoint": "process", "code": "def process(generation_json, config):\n    return generation_json"}}
  ]
}
```
...
* `post_processors` is optional. If provided as an array, processors execute sequentially (chain), where each processor receives the previous processor output as input.
